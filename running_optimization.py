...
def build_report(result):
    ...
    lines.append("FMA / IX TRACKING PLOTS")
    lines.append("-" * 90)
    for stage in ("start", "end"):
        diagnostic = result.get(f"fma_{stage}")
        if diagnostic is None:
            continue
        lines.append(
            f"{stage} FMA: "
            f"{Path(diagnostic['frequency_plot_path']).resolve()}"
        )
        lines.append(
            f"{stage} Ix:  "
            f"{Path(diagnostic['ix_plot_path']).resolve()}"
        )
    lines.append("=" * 90)
...
def main():
    ...
    fma_start = None
    if cfg.RUN_FMA_START_END:
        import FMA
        start_details_for_tracking = opt.full_diagnostics(
            context,
            cfg.GRADIENT_WEIGHT,
        ...
        fma_start = FMA.main(
            ...
            ix_vector=start_details_for_tracking["Ix"],
            ix_state=context["state"],
            stage="start",
        )
    ...
    result = opt.hybrid_optimize(
        ...
        sigma=cfg.CMA_SIGMA,
        scales=cfg.SCALES,
        cma_time=cfg.CMA_TIME,
        popsize=cfg.CMA_POPSIZE,
        print_every=cfg.PRINT_EVERY,
        powell_time_fraction=cfg.POWELL_TIME_FRACTION,
    ...
    if cfg.RUN_FMA_START_END:
        ...
        fma_end = FMA.main(
            ...
            ix_vector=result["final_details"]["Ix"],
            ix_state=result["context"]["state"],
            stage="end",
        )
